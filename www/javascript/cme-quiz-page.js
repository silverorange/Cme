...
(function () {
	...
	proto.initQuestions = function () {
		...
		for (var i = 0; i < this.question_els.length; i++) {
			form_field = Dom.getNextSibling(
				Dom.getFirstChild(this.question_els[i])
			);
	...
	proto.stopKeyboardEvent = function (e) {
		...
		if (
			target.tagName.toLowerCase() === "input" &&
			(Dom.getAncestorByClassName(target, "quiz-page") !== null ||
				Dom.getAncestorByClassName(target, "quiz-question-dialog") !==
					null)
		) {
			Event.stopEvent(e);
		}
	...
	proto.drawIntroPage = function () {
		...
		Event.on(
			...
			function (e) {
				...
				var last_completed = this.validate(
					this.question_els.length - 1
				);
	...
	proto.drawReviewPage = function () {
		...
		for (var i = 0; i < this.question_els.length; i++) {
			...
			change_button.className =
				"btn btn-default btn-xs quiz-review-change";
	...
	proto.updateIntroPage = function () {
		while (this.intro_status_line.firstChild) {
			this.intro_status_line.removeChild(
				this.intro_status_line.firstChild
			);
		}
	...
	proto.updateReviewPage = function () {
		...
		if (unanswered.length === 0) {
			...
			if (unanswered.length === 1) {
				...
				var parts = CMEQuizPage.review_status_text_2_to_5.split(
					/%s/,
					2
				);
				this.review_status.appendChild(
				...
				for (var i = 0; i < unanswered.length; i++) {
					...
					(function () {
						...
						Event.on(
							...
							function (e) {
								Event.preventDefault(e);
								that.dialog.config.toggle_element =
									Event.getTarget(e);
				...
				this.review_status.appendChild(
					document.createTextNode(
						CMEQuizPage.review_status_text_many.replace(
							/%s/,
							unanswered.length
						)
					)
	...
	proto.saveResponseValue = function (el, question_index) {
		...
		var list = Dom.getFirstChild(
			Dom.getFirstChild(
				Dom.getNextSibling(
					Dom.getFirstChild(this.question_els[question_index])
				)
			)
		);
